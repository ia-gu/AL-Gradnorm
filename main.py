...
##
# Data
logging.basicConfig(level=logging.ERROR, filename='./1000to10000.txt', format='%(message)s')
...
cifar10_train = CIFAR10('./dataset', train=True, download=True, transform=train_transform)    # specify data path
cifar10_unlabeled = CIFAR10('./dataset', train=True, download=True, transform=test_transform)
cifar10_test = CIFAR10('./dataset', train=False, download=True, transform=test_transform)
...
def train_epoch(models, criterion, optimizers, dataloaders, epoch):
    ...
    global iters
    loop = tqdm(dataloaders['train'], unit='batch', desc='| Train |', dynamic_ncols=True)
    for data in loop:
...
def train(models, criterion, optimizers, schedulers, dataloaders, num_epochs, cycle):
    ...
    for epoch in range(num_epochs):
        ...
        if epoch % 20 == 0 or epoch == 199:
            ...
            logging.error('Cycle:' + str(cycle) + 'Epoch:' + str(epoch) + '---' + 'Val Acc: {:.3f} \t Best Acc: {:.3f}'.format(acc, best_acc))
    print('>> Finished.')

#
def compute_gradnorm(models, loss):
    grad_norm = torch.tensor([]).to(device)
    gradnorm = 0.0
...
if __name__ == '__main__':
    ...
    for trial in range(TRIALS):
        ...
        for cycle in range(CYCLES):
            backbone_net = resnet.ResNet18(NUM_CLASS)
            backbone_net = nn.DataParallel(backbone_net, [0]).to(device)
            models = {'backbone': backbone_net}
            backbone_net = backbone_net.module
        ...
        torch.save({
                    'trial': trial + 1,
                    'state_dict_backbone': models['backbone'].module.state_dict()
                },
